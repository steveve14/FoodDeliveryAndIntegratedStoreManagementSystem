...
> **인증 방식**: Gateway 필터를 통해 `Authorization: Bearer <accessToken>` 헤더를 검증합니다.
> `🔓 인증 불필요` / `🔐 인증 필요 (Bearer 토큰)` 로 구분합니다.
...
Response: `data: TokenResponse` (1-1과 동일)
...
> 🔐 인증 필요
> ⚠️ Gateway가 JWT에서 추출한 `X-User-Id` 헤더를 자동으로 전달합니다. (프론트에서 직접 세팅 불필요)
...
> `userId`, `address` 필수 / `totalAmount` 0 이상 / `items` 최소 1개 / `quantity` 1 이상
