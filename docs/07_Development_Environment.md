...
### 9.1 Gateway 우회 테스트 금지 (개발 정책)

각 서비스 내부 HTTP 포트(`8010`, `8020`, `8030`, `8040`, `8050`)는 **로컬 개발 및 디버그 전용**입니다.

- QA/CI 기준 기능 테스트는 반드시 **Gateway(`http://localhost:8000`)를 통해** 수행합니다.
- 직접 포트 접근은 서비스별 동작 확인이나 gRPC 디버그 등 개발 목적에 한해 허용합니다.
- 직접 포트로 수행한 테스트 결과를 게이트웨이 통과 기준으로 간주하지 않습니다.

### 9.2 TOKEN_SECRET 로컬 기본값 안내

각 서비스 `application.yml`에 로컬 개발용 `TOKEN_SECRET` 기본값이 포함되어 있어, 환경변수 미설정 시에도 로컬에서 기동이 가능합니다.

- 기본값은 `service-auth`의 개발 전용 키와 동일하게 맞춰 서비스 간 JWT 검증 호환성을 유지합니다.
- **운영 환경에서는 반드시 `TOKEN_SECRET` 환경변수를 직접 주입해야 합니다.** 기본값 그대로 운영 배포 시 보안 위험이 발생합니다.
- 대상 서비스: `service-gateway`, `service-user`, `service-store`, `service-order`, `service-delivery`, `service-event`
