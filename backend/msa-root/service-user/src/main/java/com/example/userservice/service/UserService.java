...
public class UserService {
    ...
    @Transactional
    public UserProfileDto updateProfile(Long id, UpdateProfileRequest req) {
        User user = userRepository.findById(id).orElseThrow(() -> new NoSuchElementException("사용자를 찾을 수 없습니다."));
    ...
    public com.example.userservice.dto.AuthUserDto authenticate(String email, String rawPassword) {
        return userRepository.findByEmail(email)
                ...
                .map(u -> new com.example.userservice.dto.AuthUserDto(u.getId(), u.getEmail(), u.getName(), u.getRoles()))
                .orElse(null);
    }
}
