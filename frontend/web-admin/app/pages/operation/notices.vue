...
/**
 * [운영 > 공지사항]
 * Vue 핵심 기능과 UI 라이브러리, 테이블 코어 유틸을 불러옵니다.
 */
...
// ==========================================
// h() 함수에서 사용하기 위해 컴포넌트를 미리 resolve 합니다.
const UButton = resolveComponent('UButton');
const UBadge = resolveComponent('UBadge');
const UDropdownMenu = resolveComponent('UDropdownMenu');
const UCheckbox = resolveComponent('UCheckbox');
const UIcon = resolveComponent('UIcon');
const UAvatar = resolveComponent('UAvatar'); // [추가] 리스트 썸네일 이미지 표시

const toast = useToast();
type TableInstance<T> = { tableApi?: Table<T> } | null;
type TableRow<T> = Row<T>;
const table = ref<TableInstance<NoticeItem>>(null); // 테이블 인스턴스 참조
...
const formSchema = z.object({
  ...
  imageUrl: z.string().optional(), // [추가] 이미지 필드
});
...
// 폼 상태
const fileInput = ref<HTMLInputElement | null>(null); // 파일 입력 참조
...
function onFileSelect (event: Event) {
  ...
  if (input.files && input.files[0]) {
    ...
    // 브라우저 메모리에 임시 URL 생성 (실무에서는 업로드 API 필요)
...
function removeImage () {
  formState.imageUrl = '';
  if (fileInput.value) { fileInput.value.value = ''; } // 입력 초기화
}
...
const columns: TableColumn<NoticeItem>[] = [
  ...
  // 5. 상태 컬럼 - [수정] 필터 로직 개선
  {
    accessorKey: 'status',
    header: '상태',
    // [중요] 기본 includes 대신 equals를 사용하여 정확한 상태 필터링
    filterFn: 'equals',
    cell: ({ row }) => {
